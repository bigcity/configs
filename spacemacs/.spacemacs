...
values."
  ...
  (setq-default
   ...
   dotspacemacs-default-font (if (find-font (font-spec :name "Consolas"))
                                 '("Consolas" :size 14 :weight normal :width normal :powerline-scale 1.1)
                                 '("Monospace" :size 14 :weight normal :width normal :powerline-scale 1.1))
   ;; The leader key
...
you should place your code here."
  ...
  (c-add-style "tlael" '("stroustrup"
						 (c-offsets-alist
						  ;; Don't force function arguments to be after paren
                          (arglist-cont-nonempty . +)
                          (arglist-cont . +)
                          ;; Don't do fancy things with comments
                          (comment-intro . 0)
						  ;; Don't indent inside namespace
						  (innamespace . 0))))
  (add-hook 'c++-mode-hook (lambda ()
							 (define-key evil-insert-state-map (kbd "C-i") 'self-insert-command)
										; (c-set-style "whitesmith-4")
							 (c-set-style "tlael")
                             (setq tab-width 4)
                             (setq-default indent-tabs-mode t)
							 (setq evil-shift-width 4)
                             ))
  (if (string-equal system-type "gnu/linux") (setq python-shell-interpreter "python3"))
  ; (require 'rtags)
  ; (cmake-ide-setup)
  )
